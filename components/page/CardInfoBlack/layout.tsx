...
const CardInfoBlack: React.FC<ILayoutProps> = ({ children, page = 'creators', classBtnBack, classWrapInfo }) => {
    ...
    return (
        <CardInfoBlackStyled
            className={`mt-[114.5%] lg:mt-4.2 mb-5 md:mb-4 lg:mb-[7rem] w-full  lg:max-w-[55.2rem] float-right ${classWrapInfo}`}
        >
            <div className='flex justify-end'>{!isBigTabletSize && btnBack}</div>
            <div
                className={`relative rounded-[1rem] bg-gradient-category-detail-card px-2 lg:px-4 py-2.2 lg:py-3.5 text-white info`}
            >
